...
fun main(args: Array<String>) {
    ...
    val time = measureNanoTime {
        ...
        // get model of plane
        val data = getInformationAboutFlightAndSeatPrice(flights, spaceInAircraftByCondition, connection.createStatement())
        println("migration")
...
//        println(realSeatInformation)
        for (p in realSeatInformation.keys) {
            // было ли куплено по повышенной или пониженной (на основании всех остальных цен для этого рейса)
            val (planeCode, fareCondition) = p
            if (realSeatInformation[p]!!.keys.size > 1) {
                // для любого класса, у которого есть цена повыше -> есть extra_space
    ...
    println("time spent on migration is ${time / 1_000_000_000} seconds")
}

fun getAvailablePlanesAndConditions(statement: Statement): MutableList<Plane> {
    println("GET INFO ABOUT AIRPLANES")
    val sqlRequest = "SELECT aircraft_code, fare_conditions FROM bookings.seats GROUP BY aircraft_code, fare_conditions "
...
fun getInformationAboutFlightAndSeatPrice(
    flights: List<Flight>,
    planes: List<Plane>,
    statement: Statement): List<PlaneFlightSeatCost> {
    ...
    runBlocking {
        ...
        for (f in flights) {
            planes
                ...
                .forEach { planeFare ->
                    val query = "SELECT bp.flight_id, fare_conditions, seat_no, amount FROM bookings.ticket_flights " +
...
/*
* Map <
*   Pair<String, String>, Map<Int, Set<String>>
* >
* */
fun getRealSeatInformation(planes: List<Plane>, data: List<PlaneFlightSeatCost>): Map<Pair<String, String>, Map<Int, Set<String>>> {
