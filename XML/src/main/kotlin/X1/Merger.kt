...
class Merger(private val handler: PersonHandler) {
  ...
  fun mergeChildrenById(person: Person) {
  ...
  fun mergeParentById(person: Person) {
  ...
  fun mergeSiblinsById(person: Person) {
  ...
  fun mergeSpouseById(person: Person) {
    // adding spouse
  ...
  fun merge() {
    ...
    val merged =
        people.groupBy { it.name }.map { (name, people) ->
          Person(
              name = name,
          )
        }
